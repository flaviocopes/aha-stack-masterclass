...
export async function getUserEmail(pb: TypedPocketBase, request: Request) {
  pb.authStore.loadFromCookie(request.headers.get('Cookie') || '', 'pb_auth')
...
export async function isUserVerified(pb: TypedPocketBase) {
  ...
  const user = (await getUserObjectFromDb(
    pb,
    getCurrentUserId(pb) as string
  )) as UsersResponse
